...
def cli_available():
    ...
    assert app_file.exists(), "Application script " + str(app_file) + " not found for BDD step."
...
@then(parsers.parse('the output should be "{expected_output}"'))  # Escaped {expected_output}
def verify_output(expected_output, tmp_path):
    raw_stderr = tmp_path.process_result.stderr
    assert tmp_path.process_result.returncode == 0, f"CLI app exited with code {tmp_path.process_result.returncode}, stderr: {raw_stderr}"
