...
from {app_name_placeholder} import generate_greeting, main as app_main
...
def test_cli_with_name_argument(capsys):
    ...
    with patch('sys.argv', ['{app_name_placeholder}.py', '--name', 'Bob']):
        app_main()
    captured = capsys.readouterr()
    assert captured.out.strip() == "Hello, Bob!"

def test_cli_without_name_argument(capsys):
    with patch('sys.argv', ['{app_name_placeholder}.py']):
        app_main()
    captured = capsys.readouterr()
...
def test_cli_subprocess_with_name():
    result = subprocess.run(['python', '{app_name_placeholder}.py', '--name', 'Charlie'], capture_output=True, text=True, check=False)
    assert result.returncode == 0
    assert result.stdout.strip() == "Hello, Charlie!"

def test_cli_subprocess_without_name():
    result = subprocess.run(['python', '{app_name_placeholder}.py'], capture_output=True, text=True, check=False)
    assert result.returncode == 0
